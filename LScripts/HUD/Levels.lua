          ...
          if (i < 5 or Game.Difficulty < Difficulties.Trauma) and Levels[i][j][1] ~= "" then
              ...
              if status == 0 then
                  if i == 1 and j == 1 then
                      status = 1
                  elseif
                      1 < j and Game.LevelsStats[Levels[i][j - 1][1]] and
                          Game.LevelsStats[Levels[i][j - 1][1]].Finished == true
                   then
                      status = 1
                  elseif
                      1 < i and Game.LevelsStats[Levels[i - 1][table.getn(Levels[i - 1])][1]] and
                          Game.LevelsStats[Levels[i - 1][table.getn(Levels[i - 1])][1]].Finished == true
                   then
                      status = 1
                  end
              end
      ...
      for j = 1, table.getn(Levels[i]) do
          if Levels[i][j][1] == dir then
              if Levels == LevelsMain then
                  return "C" ..
                      string.format("%02d", i) .. "E" .. string.format("%02d", j) .. " - " .. Levels[i][j][2]
              else
...
function Levels_GetSketchByDir(dir)
  ...
  for i = 1, table.getn(files) do
      if
          string.find(string.lower(dir), "dm_") or string.find(string.lower(dir), "dmpcf_") or
              string.find(string.lower(dir), "ctf_") or
              string.find(string.lower(dir), "pro_")
       then
          table.insert(
              MapsMulti[1],
              {
