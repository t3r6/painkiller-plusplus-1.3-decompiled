...
function PSpectatorControler:Init()
    ...
    self._matMapViewBluePlayerIcon =
        MATERIAL.Create("Textures/team_blue.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
    self._matMapViewRedPlayerIcon =
        MATERIAL.Create("Textures/team_red.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
...
function PSpectatorControler:PostRender(delta)
    ...
    if
        MPCfg.GameMode ~= "Last Man Standing" or MPCfg.GameState == GameStates.Playing or
    ...
    if
        self.player ~= -1 and self.mode == CameraStates.Follow or
            self.player ~= -1 and self.mode == CameraStates.Auto and self.autoineyes == 1
     then
        self:SpectatorHUD()
    end
    if
        self.player ~= -1 and self.mode == CameraStates.InEyes or
            self.player ~= -1 and self.mode == CameraStates.Auto and self.autoineyes == 1 or
...
function PSpectatorControler:InterpolatePosition()
    if
        self._newCamPos.X ~= self._desiredCamPos.X or self._newCamPos.Y ~= self._desiredCamPos.Y or
            self._newCamPos.Z ~= self._desiredCamPos.Z
     then
...
function PSpectatorControler:InterpolateAngle()
    if
        self._newCamAng.X ~= self._desiredCamAng.X or self._newCamAng.Y ~= self._desiredCamAng.Y or
            self._newCamAng.Z ~= self._desiredCamAng.Z
     then
...
function PSpectatorControler:ForceSpecCheck()
    if
        (MPCfg.GameMode ~= "Last Man Standing" and MPCfg.GameMode ~= "Clan Arena" or
            MPCfg.GameState ~= GameStates.Playing and MPCfg.GameState ~= GameStates.Finished) and
            INP.Action(Actions.Jump)
     then
        INP.Reset()
        MPSTATS.Hide()
...
function PSpectatorControler:ProHud(he, ar, am1, am2, cwi, art)
    ...
    Hud:QuadRGBA(
        ...
        (768 + Cfg.HUDSize * 13 - Cfg.HUDSize * sizey) * h / 768,
        Cfg.HUDSize,
        false,
        colorh[1],
        colorh[2],
    ...
    Hud:DrawDigitsText1(
        ...
        (768 + Cfg.HUDSize * 15 - Cfg.HUDSize * sizey) * h / 768,
        string.sub(string.format("%03d", he), -3),
        0.9 * Cfg.HUDSize,
        CPlayer.HealthWarning
    )
    if art == 0 then
        Hud:QuadRGBA(
            Hud._matArmorNormalSH,
            Cfg.HUDSize * 17 * w / 1024,
            (768 + Cfg.HUDSize * 48 - Cfg.HUDSize * sizey) * h / 768,
            Cfg.HUDSize,
            false,
            0,
            0,
    ...
    Hud:DrawDigitsText1(
        Cfg.HUDSize * 52 * w / 1024,
        (768 + Cfg.HUDSize * 51 - Cfg.HUDSize * sizey) * h / 768,
        string.sub(string.format("%03d", ar), -3),
        0.9 * Cfg.HUDSize,
        CPlayer.ArmorWarning
    )
    local prcolor = {
        {
            0,
    ...
    if type(i) == "number" and i <= 7 and 1 <= i and cwi == i then
        ...
        Hud:DrawDigitsText1(
            (1024 - 117 * Cfg.HUDSize) * w / 1024,
            (768 + Cfg.HUDSize * 15 - Cfg.HUDSize * sizey) * h / 768,
            string.sub(string.format(cunumcharact[i], am1), -3),
            showinf * Cfg.HUDSize,
            cupriammolistnumwar[i]
        )
        Hud:DrawDigitsText1(
            (1024 - 117 * Cfg.HUDSize) * w / 1024,
            (768 + Cfg.HUDSize * 51 - Cfg.HUDSize * sizey) * h / 768,
            string.sub(string.format(cunumcharact[i], am2), -3),
            showinf * Cfg.HUDSize,
            cualtammolistnumwar[i]
        )
    end
end
...
function PSpectatorControler:SpectatorHUD()
    ...
    if ps then
        ...
        if Cfg.HUD_HudStyle ~= 0 then
            ...
            Hud:DrawDigitsText(
                (1024 - 118 * Cfg.HUDSize) * w / 1024,
                (768 + Cfg.HUDSize * 16 - Cfg.HUDSize * sizey) * h / 768,
                string.sub(string.format("%03d", ammo1), -3),
                0.9 * Cfg.HUDSize,
                ammo1warning
            )
            Hud:DrawDigitsText(
                (1024 - 118 * Cfg.HUDSize) * w / 1024,
                (768 + Cfg.HUDSize * 50 - Cfg.HUDSize * sizey) * h / 768,
                string.sub(string.format("%03d", ammo2), -3),
                0.9 * Cfg.HUDSize,
                ammo2warning
            )
            if currentweaponindex == 1 then
                Hud:Quad(
                    self._matAmmoOpenIcon,
                    (1024 - 62 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 12 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
                Hud:Quad(
                    self._matAmmoCloseIcon,
                    (1024 - 62 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 50 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
            ...
            elseif Currentweaponindex == 2 then
                Hud:Quad(
                    self._matAmmoIconSG,
                    (1024 - 52 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 11 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
                Hud:Quad(
                    self._matFreezerIcon,
                    (1024 - 55 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 46 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
            ...
            elseif currentweaponindex == 4 then
                ...
                Hud:Quad(
                    self._matAmmoRocketIcon,
                    (1024 - 52 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 17 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
            elseif currentweaponindex == 5 then
                Hud:Quad(
                    self._matAmmoIconED,
                    (1024 - 56 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 11 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
                Hud:Quad(
                    self._matAmmoElectroIcon,
                    (1024 - 52 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 42 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
            elseif currentweaponindex == 6 then
                Hud:Quad(
                    self._matAmmoIconRFT,
                    (1024 - 56 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 12 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                ...
                Hud:Quad(
                    self._matAmmoElectroIcon,
                    (1024 - 52 * Cfg.HUDSize) * w / 1024,
                    (768 + Cfg.HUDSize * 47 - Cfg.HUDSize * sizey) * h / 768,
                    Cfg.HUDSize,
                    false
                )
...
function PSpectatorControler:ClanArenaMessage(cames)
    ...
    HUD.PrintXY(
        (w - HUD.GetTextWidth(HUD.StripColorInfo(cames))) / 2 + 1,
        450 * h / 768 + 1,
        HUD.StripColorInfo(cames),
        fafonttypo,
        15,
        15,
        15,
...
function PSpectatorControler:DrawMapview()
    ...
    if Cfg.ConfigMapView then
        HUD.PrintXY(
            ...
            h / 1.5,
            "Scale (weapon1/2):" ..
                tostring(Mapview.globalscale) ..
                    [[

Transx (weapon3/4): ]] ..
                        tostring(Mapview.globaltranslatex) ..
                            [[

Transy (weapon5/6):]] ..
                                tostring(Mapview.globaltranslatey) ..
                                    [[

Translaterot (weapon7/8): ]] ..
                                        tostring(Mapview.globaltranslaterot * 180 / math.pi) ..
                                            [[

IMAGE Scale (weapon8/9):]] ..
                                                tostring(Mapview.imagescale) ..
                                                    [[

IMAGE Transx (weapon10/11): ]] ..
                                                        tostring(Mapview.imagetranslatex) ..
                                                            [[

IMAGE Transy (weapon12/13):]] ..
                                                                tostring(Mapview.imagetranslatey) ..
                                                                    [[
    ...
    for id, ps in Game.PlayerStats, nil do
        if ps and ps._Entity and ps._Entity ~= 0 and ps.Spectator == 0 and ps._animproc then
            ...
            if py > Mapview.imagetranslatey + Mapview.imagescale * sizey then
                Mapview.globaltranslatey =
                    (Mapview.imagetranslatey + Mapview.imagescale * sizey) / Mapview.imagescale -
                    psz * Mapview.globalscale
            end
            ...
            if MPCfg.GameMode == "Clan Arena" and MPCfg.GameState ~= GameStates.WarmUp then
                if ps.Team == Cfg.Team and Cfg.Team == 0 then
                    ...
                    HUD.DrawQuadRGBA(
                        self._matMapViewBluePlayerIcon,
                        xscale * px - xscale * 20 / 2,
                        yscale * py - yscale * 20 / 2,
                        xscale * 20,
                        yscale * 20,
                        255,
                        255,
