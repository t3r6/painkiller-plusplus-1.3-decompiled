  ...
  if Cfg.RocketFix or MPCfg.RocketFix then
      ...
      if MPCfg.GameMode == "People Can Fly" then
          ...
          Explosion(
              ...
              y,
              z,
              self.ExplosionStrength / resultfactor,
              self.ExplosionRange,
              self.ObjOwner.ClientID,
              AttackTypes.Rocket,
              self.Damage,
              resultfactor * 5
          )
...
function Rocket:ExplosionFX(entity, x, y, z, nx, ny, nz)
  ...
  if Game.GMode == GModes.SingleGame then
      ...
      for i = 1, n do
          local ke =
              AddItem(
              "KamykWybuchRakieta.CItem",
              0.6,
              Vector:New(px + FRand(-0.2, 0.2), py + FRand(-0.2, 0.2), pz + FRand(-0.2, 0.2)),
              false,
              Quaternion:New_FromEuler(FRand(0, 3.14), FRand(0, 3.14), FRand(0, 3.14))
          )
