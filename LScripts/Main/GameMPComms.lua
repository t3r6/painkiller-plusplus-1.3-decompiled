function Game:Server2ClientCommand(clientid, command, param)
  ...
  if
      (command == "CMD:PRINTSTATSALL" or command == "PK++VERSIONOKAY" or command == "CMD:STATSDUMP" or
          command == "CMD:STATS" or
          command == "PK++VERSIONOKAY" or
          command == "PK++ Authenticated.") and
          Game:IsServer()
   then
      for i, ps in Game.PlayerStats, nil do
