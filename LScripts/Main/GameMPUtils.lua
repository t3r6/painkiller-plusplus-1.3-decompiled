function Game:IsTeammate(clientid)
  ...
  if
      Game.PlayerStats[me].Spectator == 0 and Game.PlayerStats[clientid].Spectator == 0 and
          Game.PlayerStats[me].Spectator == 0 and
          Game.PlayerStats[clientid].Team == Game.PlayerStats[me].Team
   then
      return true
  end
  return false
...
function Game:CheckOvertime()
  if
      Game:IsServer() and Cfg.Overtime ~= nil and Cfg.Overtime ~= 0 and Game._TimeLimitOut ~= nil and
          MPCfg.TimeLimit ~= nil and
          MPCfg.GameState == GameStates.Playing and
          MPCfg.TimeLimit * 60 - Game._TimeLimitOut < 2
   then
...
function Game:GetTimeoutState()
  for i, o in Game.PlayerStats, nil do
      Game.PlayerStats[o.ClientID].tox, Game.PlayerStats[o.ClientID].toy, Game.PlayerStats[o.ClientID].toz =
          ENTITY.PO_GetPawnHeadPos(Game.PlayerStats[o.ClientID]._Entity)
  end
end

function Game:Timeout()
  for i, o in Game.PlayerStats, nil do
      ENTITY.PO_SetPawnHeadPos(
          Game.PlayerStats[o.ClientID]._Entity,
          Game.PlayerStats[o.ClientID].tox,
          Game.PlayerStats[o.ClientID].toy,
          Game.PlayerStats[o.ClientID].toz
      )
  end
end
...
function Game:DuelQueue()
  if Cfg.DuelQueue and Game:IsServer() and MPCfg.GameMode == "Duel" then
      ...
      if player1 ~= nil and player2 ~= nil then
          ...
          if Game.PlayerStats[player1].Score > Game.PlayerStats[player2].Score then
              ...
              Game.ConsoleMessageAll(
                  Game.PlayerStats[player1].Name ..
                      " beat " ..
                          Game.PlayerStats[player2].Name ..
                              ", " ..
                                  tostring(Game.PlayerStats[player1].Score) ..
                                      " - " .. tostring(Game.PlayerStats[player2].Score)
              )
      ...
      if loser ~= nil and maxplayer ~= nil and player1 ~= nil and player2 ~= nil then
          if
              loser ~= nil and maxplayer ~= nil and Game.PlayerStats[loser] ~= nil and
                  Game.PlayerStats[maxplayer] ~= nil
           then
              Game.ConsoleMessageAll(
                  "Swapping " .. Game.PlayerStats[loser].Name .. " with " .. Game.PlayerStats[maxplayer].Name
              )
          end
