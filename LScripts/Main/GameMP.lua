...
function Game_InterpretVariable(name, value)
  if Game:IsServer() then
      ...
      if password ~= nil and command ~= nil and Cfg.RconPass ~= nil and Cfg.RconPass ~= "" and name == "rcon" then
          if password == Cfg.RconPass then
              ...
              SendNetMethod(
                  ...
                  true,
                  true,
                  "Rcon password is incorrect. Please set with: rconpass <pass>"
              )
          end
      end
      if command ~= nil and name == "ref" then
          ...
          elseif password ~= "" then
              SendNetMethod(
                  ...
                  true,
                  true,
                  "You are not a referee or referee password incorrect"
              )
          else
              SendNetMethod(
                  Game.ConsoleMessage,
      ...
      if
          name == "HITSND" and Game._procSpec and Game._procSpec ~= nil and Game._procSpec.player ~= nil and
              Game._procSpec.player >= 0 and
              tonumber(value) == Game._procSpec.player and
              Cfg.HitSound
       then
      ...
      if
          name == "DTHSND" and Game._procSpec and Game._procSpec ~= nil and Game._procSpec.player ~= nil and
              Game._procSpec.player >= 0 and
              tonumber(value) == Game._procSpec.player and
              Cfg.KillSound
       then
...
function Game:OnMultiplayerServerTick(delta)
  ...
  if 1 <= INP.GetTime() - self.LastGameSpyReport then
      ...
      for j, pks in Game.PlayerStats, nil do
          if
              Cfg.BotQuickRespawn and pks and pks.Bot and pks._Entity == nil and
                  (MPCfg.GameState == GameStates.WarmUp or MPCfg.GameState == GameStates.Playing)
           then
              for j, pkds in Game.Players, nil do
                  if
                      pks and pkds and pks.ClientID == pkds.ClientID and pkds._timeToRespawn and
                          1 < pkds._timeToRespawn
                   then
...
function Game:OnMultiplayerClientTick(delta)
  ...
  elseif
      INP.UIAction(UIActions.Scoreboard) and
          (MPCfg.GameState == GameStates.WarmUp or
              MPCfg.GameState == GameStates.Counting and MPGameRules[MPCfg.GameMode].StartState == GameStates.WarmUp)
   then
...
function Game:NewPlayerRequest(clientID, name, model, team, state, spectator)
  ...
  if
      MPCfg.GameMode == "Last Man Standing" and
          (MPCfg.GameState == GameStates.Playing or MPCfg.GameState == GameStates.Finished)
   then
      spectator = 1
  end
  if
      MPCfg.GameMode == "Clan Arena" and
          (MPCfg.GameState == GameStates.Playing or MPCfg.GameState == GameStates.Finished or
              MPCfg.GameState == GameStates.Counting)
   then
      spectator = 1
  end
  ...
  if
      (not (Cfg.GameMode == "Duel" and Cfg.ForceSpec) or playercount ~= 2 or spectator ~= 0) and Cfg.ForceSpec and
          Game.NewComers[clientID] == nil and
          spectator == 0 and
          MPCfg.GameState == GameStates.Playing and
          (Cfg.GameMode == "Duel" or Cfg.GameMode == "Team Deathmatch" and Game.bot[clientID] == nil)
   then
...
function Game:OnNewPlayerInGame(
  clientID,
  entity,
  name,
  score,
...
function Game:PlayerSpectatorRequest(clientID, spectator)
  ...
  if
      MPCfg.GameMode == "Clan Arena" and
          (MPCfg.GameState == GameStates.Playing or MPCfg.GameState == GameStates.Finished) and
          spectator == 1
   then
      can = true
  elseif
  ...
  if
      MPCfg.TeamLock and spectator == 0 and
          (MPCfg.GameState == GameStates.Playing or MPCfg.GameState == GameStates.Finished)
   then
      can = false
...
function Game:PlayerRespawnRequest(clientID)
  if
      MPCfg.GameState ~= GameStates.Playing and MPCfg.GameState ~= GameStates.WarmUp and
          MPCfg.GameState ~= GameStates.Counting
   then
      return
  end
...
function Game:CheckWarmUpStatus()
  ...
  if Game:IsServer() then
      if
          MPCfg.GameState == GameStates.Playing and
              (pbreak >= n / 2 or
                  MPGameRules[Cfg.GameMode].Teams and Cfg.StopMatchOnTeamQuit and (blues == 0 or reds == 0))
       then
          StringToDo = "Game.EndOfMatch()"
      end
  elseif
      Game:IsServer() and Cfg.GameMode == "Clan Arena" and MPCfg.GameState == GameStates.Playing and
          (blues == 0 or reds == 0)
   then
      if reds == 0 then
  ...
  if myplayer then
      ...
      if not Cfg.NoSpawnEffects then
          AddObject(
              "FX_Spawn.CActor",
              0.6,
              Vector:New(x - fx / 3, y, z - fz / 3),
              Quaternion:New_FromNormalY(fx, 0, fz),
              true
          )
      end
...
function Game:BrightSkin(entity, enable, team)
  ...
  if
      entity and 0 < entity and not PMENU.Active() and teams and team == Cfg.Team and
          (not Player or entity ~= Player._Entity) and
          (Cfg.TeammateIcon == 1 or Cfg.TeammateIcon == 2)
   then
      ...
      if Cfg.TeammateIcon == 2 then
          ...
          if Cfg.TeammateIcon_RGB[4] == 0 then
              if
                  not (not teams or Cfg.Team ~= team or Cfg.FixedColors) or Cfg.FixedColors and teams and team == 0 or
                      not teams and Player and entity == Player._Entity
               then
  ...
  if Cfg.BrightSkins and enable and MPCfg.AllowBrightskins then
      ...
      if not PMENU.Active() then
          if not Cfg.BrightSkinsFull then
              MATERIAL.Replace(
                  "models/mp-model-fallenangel/mpplayer1texture1",
                  "models/mp-model-fallenangel/mpplayer1texture1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-fallenangel/mpplayer1texture2",
                  "models/mp-model-fallenangel/mpplayer1texture2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-fallenangel/mpplayer1texture3",
                  "models/mp-model-fallenangel/mpplayer1texture3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-demon/mp_player_2_tex_1",
                  "models/mp-model-demon/mp_player_2_tex_1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-demon/mp_player_2_tex_2",
                  "models/mp-model-demon/mp_player_2_tex_2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-beast/mpplayer5_texture_1",
                  "models/mp-model-beast/mpplayer5_texture_1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-beast/mpplayer5_texture_2",
                  "models/mp-model-beast/mpplayer5_texture_2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-beast/mpplayer5_texture_3",
                  "models/mp-model-beast/mpplayer5_texture_3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-painkiller/PAINPLAYER_texture1",
                  "models/mp-model-painkiller/PAINPLAYER_texture1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-painkiller/PAINPLAYER_texture2",
                  "models/mp-model-painkiller/PAINPLAYER_texture2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-painkiller/PAINPLAYER_texture3",
                  "models/mp-model-painkiller/PAINPLAYER_texture3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-knight/templar_tex_1",
                  "models/mp-model-knight/templar_tex_1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-knight/templar_tex_2",
                  "models/mp-model-knight/templar_tex_2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-knight/templar_tex_3",
                  "models/mp-model-knight/templar_tex_3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-player6/mpplayer6_texture1",
                  "models/mp-model-player6/mpplayer6_texture_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-player7/labcomando_texture1",
                  "models/mp-model-player7/mpplayer7_texture_brightskin"
              )
          else
              MATERIAL.Replace(
                  "models/mp-model-fallenangel/mpplayer1texture1",
                  "FullBrightSkins/mp-model-fallenangel/mpplayer1texture1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-fallenangel/mpplayer1texture2",
                  "FullBrightSkins/mp-model-fallenangel/mpplayer1texture2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-fallenangel/mpplayer1texture3",
                  "FullBrightSkins/mp-model-fallenangel/mpplayer1texture3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-demon/mp_player_2_tex_1",
                  "FullBrightSkins/mp-model-demon/mp_player_2_tex_1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-demon/mp_player_2_tex_2",
                  "FullBrightSkins/mp-model-demon/mp_player_2_tex_2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-beast/mpplayer5_texture_1",
                  "FullBrightSkins/mp-model-beast/mpplayer5_texture_1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-beast/mpplayer5_texture_2",
                  "FullBrightSkins/mp-model-beast/mpplayer5_texture_2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-beast/mpplayer5_texture_3",
                  "FullBrightSkins/mp-model-beast/mpplayer5_texture_3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-painkiller/PAINPLAYER_texture1",
                  "FullBrightSkins/mp-model-painkiller/PAINPLAYER_texture1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-painkiller/PAINPLAYER_texture2",
                  "FullBrightSkins/mp-model-painkiller/PAINPLAYER_texture2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-painkiller/PAINPLAYER_texture3",
                  "FullBrightSkins/mp-model-painkiller/PAINPLAYER_texture3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-knight/templar_tex_1",
                  "FullBrightSkins/mp-model-knight/templar_tex_1_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-knight/templar_tex_2",
                  "FullBrightSkins/mp-model-knight/templar_tex_2_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-knight/templar_tex_3",
                  "FullBrightSkins/mp-model-knight/templar_tex_3_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-player6/mpplayer6_texture1",
                  "FullBrightSkins/mp-model-player6/mpplayer6_texture_brightskin"
              )
              MATERIAL.Replace(
                  "models/mp-model-player7/labcomando_texture1",
                  "FullBrightSkins/mp-model-player7/mpplayer7_texture_brightskin"
              )
          end
      end
  else
  ...
  if Game._ChatIcon and Game._SomeConsoleOpened then
      ...
      for i, o in Game.ChatData, nil do
          if Game.PlayerStats[o.ClientID] ~= nil then
              ...
              if
                  entity and 0 < entity and not PMENU.Active() and (not Player or entity ~= Player._Entity) and
                      Game.ChatData[o.ClientID].Check and
                      Cfg.ChatIcon
               then
                  ...
                  BILLBOARD.SetupCorona(
                      ...
                      0,
                      chatstyle,
                      Color:New(255, 255, 255, 255):Compose(),
                      4,
                      true
                  )
                  ENTITY.SetPosition(ei, 0, 2.7, 0)
