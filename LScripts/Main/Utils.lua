function Clone(t)
  ...
  while i do
      if
          type(v) == "table" and not getmetatable(v) and string.sub(i, 1, 2) ~= "s_" and string.sub(i, 1, 3) ~= "Obj" and
              string.sub(i, 1, 4) ~= "_Obj" and
              string.sub(i, 1, 2) ~= "r_" and
              string.sub(i, 1, 3) ~= "_r_"
       then
          v = Clone(v)
      end
      new[i] = v
...
function Inherit(obj, t, override)
  ...
  while i do
      if
          not getmetatable(v) and type(v) == "table" and string.sub(i, 1, 3) ~= "Obj" and
              string.sub(i, 1, 4) ~= "_Obj" and
              string.sub(i, 1, 2) ~= "r_" and
              string.sub(i, 1, 3) ~= "_r_"
       then
...
function SaveTable(f, tab, name, diff, parent)
  ...
  while i do
      if not getmetatable(v) and string.sub(i, 1, 1) ~= "_" and string.sub(i, 1, 2) ~= "s_" and type(v) ~= "function" then
          ...
          if type(v) == "table" then
              if
                  string.sub(i, 1, 3) ~= "Obj" and string.sub(i, 1, 4) ~= "_Obj" and string.sub(i, 1, 2) ~= "r_" and
                      string.sub(i, 1, 3) ~= "_r_"
               then
                  local typ = "{}"
...
function SaveFullTable(f, tab, name)
  ...
  for i, v in tab, nil do
      ...
      local vtype = type(v)
      if
          (itype == "number" or itype == "string" and string.sub(i, 1, 2) ~= "s_") and vtype ~= "function" and
              not getmetatable(v)
       then
          if vtype == "table" then
...
function TraceFromCamera(length)
  ...
  local b, d, x, y, z, nx, ny, nz, he, e =
      WORLD.LineTrace(cx, cy, cz, cx + fx * length, cy + fy * length, cz + fz * length)
...
function TempObjName()
  local name =
      "Tmp" ..
      math.random(9) .. GObjects:Count() .. math.random(9) .. math.random(9) .. math.random(9) .. math.random(9)
  if _G[name] ~= nil then
      return TempObjName()
  end
  return name
...
function PreloadTemplates(path, leveltemplates)
  ...
  for i = 1, table.getn(dirs) do
      if
          Game.GMode == GModes.SingleGame or
              Game.GMode ~= GModes.SingleGame and not leveltemplates and dirs[i] ~= "Monsters" and dirs[i] ~= "Items" and
                  dirs[i] ~= "Treasures"
       then
...
function CanBurning(obj)
  return obj and obj.OnDamage and not obj.notBleeding and not obj.DisableFreeze and
      (obj._Class ~= "CItem" or not (not obj.DestroyPack))
end
