...
function CPlayer:ClientTick(delta)
  ...
  if checkSwitchedFire and (IsBitFlag(action, Actions.Fire) or IsBitFlag(action, Actions.AltFire)) then
      ...
      if
          not (not Cfg.SwitchFire[curSlot] or Game.SwitchFire[curSlot]) or
              Game.SwitchFire[curSlot] and not Cfg.SwitchFire[curSlot]
       then
  ...
  if
      (not (cw and cw._zoom) or not (0 < cw._zoom)) and
          (INP.Action(Actions.NextWeapon) or INP.Action(Actions.PrevWeapon))
   then
  ...
  if cw then
      if
          not IsFinalBuild() and INP.Key(Keys.G) == 2 or Game.TPP or Cfg.ViewWeaponModel == false or
              cw._zoom and 0 < cw._zoom
       then
  ...
  if cw then
      if
          Game.Active and Game.GMode == GModes.MultiplayerClient and
              (nextSlot == -1 or nextSlot == self._CurWeaponIndex and cw._ActionState == "Idle")
       then
...
function CPlayer:TryToChangeWeapon(slot)
  ...
  if
      ENTITY.PO_IsActionState(self._Entity, Actions.FireBestWeapon1) or
...
function CPlayer:InterpretAction(delta)
  ...
  if
      prevWeap == self._CurWeaponIndex and not ENTITY.PO_IsActionState(self._Entity, Actions.FireBestWeapon1) and
          not ENTITY.PO_IsActionState(self._Entity, Actions.FireBestWeapon2) and
          self._SwitchToWeapon and
          not self._frjump
   then
  ...
  if cw then
      if
          cw.ShotTimeOut <= 0 and not self._frjump and
              (ENTITY.PO_IsActionState(self._Entity, Actions.ForwardRocketJump) or
                  ENTITY.PO_IsActionState(self._Entity, Actions.RocketJump)) and
              cw.BaseObj == "MiniGunRL.CWeapon"
       then
          self._frjump = 0
      end
...
function CPlayer:ClientTick2(delta)
  ...
  if
      Game._EarthQuakeProc and Game._EarthQuakeProc._noEquake and
          (self._CurWeaponIndex ~= 6 or not self:GetCurWeapon()._altfire)
   then
...
function CPlayer:Update()
  if not self._died then
      ...
      if
          Game.GMode == GModes.SingleGame and self.Health < 75 and 10 < INP.GetTime() - self._lastTimeHitInc and
              Game.HealthRegen and
              INP.GetTime() - self._lastHealthInc >= 0.3
       then
          self.Health = self.Health + 1
          self._lastHealthInc = INP.GetTime()
      end
      if
          Game.GMode == GModes.SingleGame and 75 > self.Armor and self.ArmorType ~= 0 and
              10 < INP.GetTime() - self._lastTimeHitInc and
              Game.ArmorRegen and
              INP.GetTime() - self._lastHealthInc >= 0.3
       then
...
function CPlayer:OnDamage(damage, killer, attack_type, x, y, z, nx, ny, nz)
  ...
  if
      (not (not MPCfg.ProPlus and Cfg.FallingDamage) or MPCfg.GameMode == "Clan Arena" or MPCfg.GameMode == "Instagib" or
          MPCfg.GameMode == "ICTF") and
          MPCfg.GameState == GameStates.Playing and
          attack_type == AttackTypes.HitGround
   then
      return
  end
  ...
  if self.HasPentagram and attack_type ~= AttackTypes.OutOfLevel and attack_type ~= AttackTypes.ConsoleKill then
      self._score = nil
      if Game.GMode ~= GModes.SingleGame then
          self.Client_OnDamage(
              self.ClientID,
              self._Entity,
              self.Health,
              self.Armor,
              AttackTypes.Hurt_Pent,
              damage,
              kID
          )
      end
  ...
  if
      GOD and (not IsFinalBuild() or attack_type ~= AttackTypes.OutOfLevel) or
          not (not Game.IsDemon or Lucifer_001) and attack_type ~= AttackTypes.OutOfLevel or
          Game.PlayerNoDamage == true and attack_type ~= AttackTypes.OutOfLevel
   then
      self._score = nil
      return
  end
  ...
  elseif attack_type ~= AttackTypes.OutOfLevel and attack_type ~= AttackTypes.TeleFrag then
      ...
      if
          MPGameRules[MPCfg.GameMode].Teams and not MPCfg.TeamDamage and killer and
  ...
  if not self._died then
      ...
      if self.Health <= 0 then
          if
              Game.GMode == GModes.SingleGame and Game.PlayerRegenerateWhenDying == true and
                  attack_type ~= AttackTypes.OutOfLevel
           then
              self.Health = 33
      ...
      if self.Health > 0 then
          ...
          if Game.GMode ~= GModes.SingleGame then
              ...
              if
                  MPCfg.GameState == GameStates.Playing and MPCfg.GameMode ~= "Clan Arena" and
...
function CPlayer:Steps(delta)
  ...
  if
      Game.CameraFromPlayer and PLAYER.FloorCheck(self._Entity) and
          (INP.Action(2) or INP.Action(4) or INP.Action(8) or INP.Action(16))
   then
  ...
  if player then
      ...
      if player == Player then
          ...
          if Game.GMode == GModes.SingleGame and (not player._oldSND or not SOUND2D.IsPlaying(player._oldSND)) then
              if
                  attack_type and attack_type == AttackTypes.AIFar and
                      player._lastTimePlayedSoundHit + player.minTimeBetweenHit < Game.currentTime
               then
...
function CPlayer:Common_UpdateStats(checkOnly, deadID, killerID, attack_type, score)
  ...
  if ds then
      ...
      if ks and ks ~= ds then
          if
              MPGameRules[MPCfg.GameMode].Teams and (attack_type == AttackTypes.TeleFrag or MPCfg.TeamDamage) and
                  ks.Team == ds.Team
           then
              ks.Score = ks.Score - 1
              ks.Kills = ks.Kills + 1
          ...
          if ds and ds.Deaths >= 1 then
              ...
              if attack_type == 4 or attack_type == 7 or attack_type == 28 then
                  SendNetMethod(
                      ...
                      true,
                      ServerID,
                      "You are proven that life is short.",
                      0
                  )
              end
  ...
  if checkOnly then
      ...
      if Cfg.PitabotEnabled then
          if
              ks and ds and ds ~= ks and ds.Name ~= nil and attack_type ~= nil and ks.Name ~= nil and ds.Score ~= nil and
                  ks.Score ~= nil
           then
              PBLogEvent(
                  ds.Name,
...
function CPlayer:Client_OnDeath(deadID, killerID, attack_type, gib, score, damage)
  ...
  if
      Player and Player.ClientID == killerID and deadID ~= Player.ClientID and Cfg.HUD_FragMessage and
      ...
      if not Cfg.LowQualityMultiplayerSFX then
          if gib == 0 then
              ...
              if not IsDedicatedServer() then
                  ...
                  local x, y, z =
                      MDL.TransformPointByJoint(deadEntity, MDL.GetJointIndex(deadEntity, "k_chest"), 0, 0, 0)
                  if not Cfg.NoGibs then
